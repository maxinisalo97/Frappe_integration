...
public static function generar_pdf_conjunto_usuario($courseid, $username) {
    ...
    // 3) Recoger todos los attempt IDs "prueba" de ese usuario en el curso
    ...
    // 4) Obtener el instanceid de nuestro bloque dedication_atu en este curso
    ...
    // 5) Montar $_GET para invocar dedication_atu.php en modo PDF conjunto
    $_GET = [
        'courseid'   => $params['courseid'],
        'instanceid' => $instanceid,
        'userid'     => $user->id,               // <-- ¡Este es el cambio clave!
        'task'       => 'pdf_conjunto_pruebas',
        'modo_pdf'   => 1,
        'attemptid'  => array_map(function($r) { return $r->attemptid; }, $rs),
    ];

    // 6) Inicializar Moodle, contexto y login de curso
    require_once($CFG->dirroot . '/config.php');
    $course = $DB->get_record('course', ['id' => $params['courseid']], '*', MUST_EXIST);
    require_course_login($course);

    // 7) Capturar la salida PDF que genera dedication_atu.php
    ob_start();
    require_once($CFG->dirroot . '/blocks/dedication_atu/dedication_atu.php');
    $pdfraw = ob_get_clean();

    // 8) Devolverlo en Base64
    return [
        'status'  => 'success',
